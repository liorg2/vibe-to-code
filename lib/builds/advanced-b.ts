...
/** Build track, steps 18–24 (Advanced, second half): from a fast, scaled app to a production-grade one. */
export const BUILDS_ADV_B: Record<string, BuildStep> = {
  net: {
    ...
    uses: ["domain-and-registrar", "tls-certificate", "is-it-me-or-them", "https-tls"],
...
Report back in plain words: what you checked, pass or fail, with the real output below. If anything fails, stop and explain it simply; don't fix it silently. If all is green, add a line to STEPS.md ("18 net: one https address, smoke test, site-down guide") and save the work with a commit.`,
    done: [
      {
...
Report back in plain words: what you checked, pass or fail, with the real output below. If anything fails, above all a photo anyone can open, stop and explain it simply; don't fix it silently. If all is green, add one line to STEPS.md ("19 cloud: private contact photos, map of services and costs") and commit.`,
    done: [
      {
  ...
  devops: {
    ...
    uses: ["deploy", "preview-deployment", "environments-dev-staging-prod", "feature-flag", "rollback"],
...
Report back in plain words: what you checked, pass or fail, with the real output below. If anything fails, stop and explain it simply; don't fix it silently. If all is green, add a line to STEPS.md ("20 devops: CI blocks red changes, preview databases, deal-value flag, rollback practised") and commit.`,
    done: [
      {
  ...
  observe: {
    ...
    uses: ["logs-and-monitoring", "tracing-and-correlation-id", "error-tracking", "alert", "pageview-vs-event"],
...
Report back in plain words: what you checked, pass or fail, with the real output below. If anything fails, stop and explain it simply; don't fix it silently. If all is green, add a line to STEPS.md ("21 observe: logs with request ids, Sentry, health check, uptime alert, 4 private events") and commit.`,
    done: [
      {
  ...
  team: {
    ...
    uses: ["readme-and-docs", "ticket-issue", "bus-factor"],
...
Report back in plain words: what you checked, pass or fail, with the real output below. If the stranger test hit a gap, stop and list the gaps; don't quietly patch the README. If all is green, delete the fresh copy, add one line to STEPS.md ("22 team: README, decision record, changelog, PR template, next ticket") and commit.`,
  ...
  llm: {
    ...
    uses: ["llm-api-call", "system-prompt", "streaming-response", "environment-variables-secrets", "unit-integration-e2e", "invalidation", "timeouts-and-retries"],
...
Report back in plain words: what you checked, pass or fail, with the real output below. If anything fails, stop and explain it simply; don't fix it silently. If all is green, add a line to STEPS.md ("23 llm: AI contact summary, server-only key, streamed, cost logged, cached until notes change") and commit.`,
  ...
  ai: {
    ...
    uses: ["agents-md", "prompt-injection", "environment-variables-secrets", "rate-limiting", "make-it-verify-itself", "vibe-coding-vs-vibe-engineering"],
