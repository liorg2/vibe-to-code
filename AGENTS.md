...
- **`stg`** is the staging branch. It trails `main` and carries nothing of its own, so ship to it with a fast-forward: `git push origin main:stg`. Never merge `stg` into `main`. Its branch link is https://vibe-to-code-git-stg-lior-goldembergs-projects.vercel.app (a Preview, so it sits behind the Vercel login gate).
