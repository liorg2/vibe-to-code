...
function termProgress(ids: string[], done: Set<string>) {
  ...
  return { total, d, pct: total ? Math.round((d / total) * 100) : 0 };
}
...
export function CourseProgress({ ids }: { ids: string[] }) {
  const { done, t } = useApp();
  const { total, d, pct } = termProgress(ids, done);
  return (
    <div>
      <Progress value={pct} className={cn(miniBar)} />
      <span className="cnt">{d}/{total} {t("terms")}</span>
    </div>
  );
}

/** Course-card link. Owners who have not opened a topic yet get "Start learning", then "Continue learning · 40%". */
export function CourseOpen({ ids, href, owned, className }: { ids: string[]; href: string; owned: boolean; className: string }) {
  ...
  const { d, pct } = termProgress(ids, done);
  const built = ids.some((id) => buildFinished(ticked, id));
  const label = owned ? (d || built ? `${t("continue")} · ${pct}%` : t("start")) : t("viewLessons");
  return (
